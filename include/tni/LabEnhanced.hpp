#ifndef TNI_API_HEADER_LABENHANCED
#define TNI_API_HEADER_LABENHANCED
...
struct LabEnhanced : public Node2D {
	using Node2D::Node2D;

	constexpr LabEnhanced(Node2D base) : Node2D{base} {}
	constexpr LabEnhanced(uint64_t addr) : Node2D{addr} {}
	constexpr LabEnhanced(Object obj) : LabEnhanced{obj.address()} {}
	LabEnhanced(Variant variant) : LabEnhanced{variant.as_object().address()} {}
...
inline Variant LabEnhanced::is_valid_global_point(Variant gpoint) { return operator()("is_valid_global_point", gpoint); }
inline Variant LabEnhanced::get_floor_idx_from_global_point(Variant gpoint) { return operator()("get_floor_idx_from_global_point", gpoint); }
inline Variant LabEnhanced::debug_monitor_callback() { return operator()("debug_monitor_callback"); }
inline void LabEnhanced::add_link_outlet(LinkOutlet newsock, Variant sockpos) { voidcall("add_link_outlet", newsock, sockpos); }
inline void LabEnhanced::populate_bill_breakdown(Variant payment_breakdown, bool power_fee_waiver, double global_network_fee_mult) { voidcall("populate_bill_breakdown", payment_breakdown, power_fee_waiver, global_network_fee_mult); }
