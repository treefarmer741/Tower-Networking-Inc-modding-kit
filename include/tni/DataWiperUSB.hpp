...
inline PackedScene DataWiperUSB::get_cable_make_scene() { return PackedScene(this->operator()("get_cable_make_scene").as_object().address()); }
inline void DataWiperUSB::set_highlight(bool enabled) { this->voidcall("set_highlight", enabled); }
inline void DataWiperUSB::apply_color(Variant color_val) { this->voidcall("apply_color", color_val); }
inline void DataWiperUSB::apply_label(String text, Variant color, bool labelled) { this->voidcall("apply_label", text, color, labelled); }
